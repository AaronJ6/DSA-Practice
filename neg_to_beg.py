def neg_beg(arr): #!MY CODE Time - O(n); Space - O(1)
    i, j = 0, 0
    while j<len(arr):
        if arr[j] >= 0:
            j+=1
        elif arr[j] < 0:
            arr[i],arr[j] = arr[j],arr[i]
            j+=1
            i+=1
    print(arr)

def dutch_neg_beg(arr): #! G4G method, same complexity [Time - O(n); Space - O(1)] BUT doesnt maintain the order of elements like mine for neg no.s
    low, mid = 0,0
    high = len(arr) - 1
    while(mid<=high):
        if arr[mid] < 0:
            arr[low], arr[mid] = arr[mid], arr[low]
            low+=1
            mid+=1
        elif arr[mid] > 0:
            arr[high], arr[mid] = arr[mid], arr[high]
            high-=1
        else:
            mid+=1

# Python3 program to Rearrange positive
# and negative numbers in an array without losing relative position and #!NO extra data structure only recursive calls O(lgn) is used and Time comeplexity - O(nlgn)

# Function to reverse an array. An array can
# be reversed in O(n) time and O(1) space.
def reverse(arr, l, r):

	if l < r:

		arr[l], arr[r] = arr[r], arr[l]
		l, r = l + 1, r - 1
		reverse(arr, l, r)


# Merges two subarrays of arr[].
# First subarray is arr[l..m]
# Second subarray is arr[m + 1..r]
def merge(arr, l, m, r):

	i = l # Initial index of 1st subarray
	j = m + 1 # Initial index of IInd

	while i <= m and arr[i] < 0:
		i += 1

	# arr[i..m] is positive

	while j <= r and arr[j] < 0:
		j += 1

	# arr[j..r] is positive

	# reverse positive part of left
	# sub-array (arr[i..m])
	reverse(arr, i, m)

	# reverse negative part of right
	# sub-array (arr[m + 1..j-1])
	reverse(arr, m + 1, j - 1)

	# reverse arr[i..j-1]
	reverse(arr, i, j - 1)


# Function to Rearrange positive
# and negative numbers in a array
def RearrangePosNeg(arr, l, r):

	if l < r:

		# Same as (l + r)/2, but avoids
		# overflow for large l and h
		m = l + (r - l) // 2

		# Sort first and second halves
		RearrangePosNeg(arr, l, m)
		RearrangePosNeg(arr, m + 1, r)

		merge(arr, l, m, r)


def main():
    nums = [-12, 11, -13, -5, 6, -7, 5, -3, -6]
    print(nums)
    dutch_neg_beg(nums)
    print(nums)
    # for k in range(len(op)):
    #     print(op[k])


if __name__ == "__main__":
    main()

